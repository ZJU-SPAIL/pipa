...
from pipa.common.utils import file_format, check_file_format
...
def test_check_file_format_xz(mock_open):
    assert check_file_format(file="test.xz") == file_format.xz
...
def test_check_file_format_bzip2(mock_open):
    assert check_file_format(file="test.xz") == file_format.bzip2
    mock_open.assert_called_with("test.xz", "rb")
...
def test_check_file_format_elf(mock_open):
    assert check_file_format(file="test.xz") == file_format.elf
    mock_open.assert_called_with("test.xz", "rb")
...
def test_check_file_format_other(mock_open):
    assert check_file_format(file="test.xz") == file_format.other
    mock_open.assert_called_with("test.xz", "rb")
