...
# 确保动态库路径在子 Shell 中也生效
export LD_LIBRARY_PATH="$MYSQL_INSTALL_DIR/lib:$LD_LIBRARY_PATH"

echo "   -> 启动负载: threads=$SYSBENCH_THREADS, affinity=$SYSBENCH_CPU_AFFINITY"
taskset -c "$SYSBENCH_CPU_AFFINITY" "$SYSBENCH_INSTALL_DIR/bin/sysbench" "$RUN_LUA_SCRIPT" \
    --mysql-socket="$MYSQL_SOCKET_PATH" --mysql-user=root \
    --tables="$SYSBENCH_TABLES" --table-size="$SYSBENCH_TABLE_SIZE" \
    --report-interval=1 --threads="$SYSBENCH_THREADS" --time=300 run > "$EVIDENCE_DIR/sysbench.log" 2>&1 &
SYSBENCH_PID=$!
...
if ! kill -0 $SYSBENCH_PID 2>/dev/null; then
    ...
    echo "--- 环境变量诊断 ---"
    env | grep -E "LD_LIBRARY_PATH|MYSQL"
    echo "--- 日志尾部内容 ---"
    tail -n 20 "$EVIDENCE_DIR/sysbench.log"
    "$SCRIPT_DIR/stop_mysql_graceful.sh"
    exit 1
fi

pipa -vv sample --attach-to-pid "$MYSQL_PID" --duration-stat 60 --duration-record 60 --output "$EVIDENCE_DIR/snapshot.pipa" --static-info-file "$PROJECT_ROOT/pipa_static_info.yaml"
