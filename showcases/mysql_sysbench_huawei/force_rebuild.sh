...
taskset -c "$SYSBENCH_CPU_AFFINITY" env LD_LIBRARY_PATH="$MYSQL_INSTALL_DIR/lib" "$SYSBENCH_INSTALL_DIR/bin/sysbench" "$PREPARE_LUA_SCRIPT" --mysql-socket="$MYSQL_SOCKET_PATH" --mysql-user=root --tables="$SYSBENCH_TABLES" --table-size="$SYSBENCH_TABLE_SIZE" --threads="$SYSBENCH_THREADS" prepare
