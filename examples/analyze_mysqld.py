...
def main() -> None:
    ...
    write_symbol_stats_csv(os.path.join(OUT_DIR, "mysqld_hot_symbols.csv"), hot_syms, total=an.total_weight)
    ...
    write_stack_stats_csv(os.path.join(OUT_DIR, "mysqld_hot_stacks.csv"), hot_stacks, total=an.total_weight)
    ...
    write_symbol_stats_csv(os.path.join(OUT_DIR, "mysqld_hot_symbols_focus.csv"), hot_syms_focus, total=an.total_weight)

    # Save top callchains text for inspection
    with open(os.path.join(OUT_DIR, "mysqld_hot_callchains.txt"), "w", encoding="utf-8") as f:
    ...
    if hot_syms:
        ...
        with open(os.path.join(OUT_DIR, "mysqld_trie_subtree.json"), "w", encoding="utf-8") as f:
