...
- [**PIPA 权威使用手册 (v1.0)**](#pipa-权威使用手册-v10)
  - [**目录**](#目录)
    - [**1. 核心哲学：性能分析的“CT 扫描仪”** ](#1-核心哲学性能分析的ct-扫描仪-)
    - [**2. 安装与环境准备** ](#2-安装与环境准备-)
      - [**2.1 系统依赖**](#21-系统依赖)
      - [**2.2 安装 PIPA**](#22-安装-pipa)
    - [**3. 黄金工作流：十分钟定位性能问题** ](#3-黄金工作流十分钟定位性能问题-)
      - [**场景**: 你的应用（进程 PID 为 `25188`）在运行时出现性能下降，你想找出原因。](#场景-你的应用进程-pid-为-25188在运行时出现性能下降你想找出原因)
      - [**第 1 步: 系统健康检查 (可选但推荐)**](#第-1-步-系统健康检查-可选但推荐)
      - [**第 2 步: 执行性能快照 (`sample`)**](#第-2-步-执行性能快照-sample)
      - [**第 3 步: 智能分析与生成报告 (`analyze`)**](#第-3-步-智能分析与生成报告-analyze)
      - [**第 4 步: (如果需要) 深度代码剖析 (`flamegraph`)**](#第-4-步-如果需要-深度代码剖析-flamegraph)
    - [**4. 命令参考大全** ](#4-命令参考大全-)
    ...
    - [**5. 解读分析报告** ](#5-解读分析报告-)
    - [**5.1 交互式筛选器：从数据中掘金**](#51-交互式筛选器从数据中掘金)
      - [**1. 基础筛选 (多选)**](#1-基础筛选-多选)
      - [**2. 范围筛选 (数值型)**](#2-范围筛选-数值型)
      - [**3. 高级筛选 (精确匹配)**](#3-高级筛选-精确匹配)
      - [**4. 组合筛选 (终极用法)**](#4-组合筛选-终极用法)
    - [**5.2 磁盘拓扑与容量分析** ](#52-磁盘拓扑与容量分析-)
...
- **Analysis & Insights**:
  ...
  - **诊断结论**: 基于决策树的最终文字总结，明确指出系统瓶 LINGJING 类型和可能的根因。诊断引擎现在也支持对 NUMA 架构下的跨节点不均衡问题进行检测。**这是你首先要看的部分**。
...
### **5.2 磁盘拓扑与容量分析** <a name="52-磁盘拓扑与容量分析"></a>

除了时序指标，PIPA 报告还提供了对系统磁盘物理拓扑和使用率的可视化分析，帮助您快速识别容量风险。

![磁盘拓扑图](/docs/images/Disk-Topology.png)

上图（截图 A）是对磁盘布局的旭日图（Sunburst Chart）展示：

- **层级结构**: 图表中心的内环代表**物理磁盘** (如 `sda`, `nvme0n1`)，外环代表其上的**逻辑分区** (如 `sda1`, `sda2`)。此视图支持点击内环或外环进行放大和聚焦。
- **颜色含义**:
  - <span style="color:green;">■</span> **绿色**: 代表高性能磁盘，如 SSD 或 NVMe。
  - <span style="color:blue;">■</span> **蓝色**: 代表传统机械硬盘 (HDD)。
  - <span style="color:purple;">■</span> **紫色**: 代表逻辑卷管理 (LVM) 设备。
- **警告机制**: 当任何一个分区的**使用率超过 90%** 时，其标签会以醒目的**红色**高亮显示，提示容量风险。
