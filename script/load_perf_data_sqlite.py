...
def usage():
    printerr(
        "Usage is: export-to-sqlite.py <database name> [<columns>] [<calls>] [<callchains>] [<csv[=dir>]]"
    )
...
columns = "all"
arg_start = 2
if len(sys.argv) >= 3 and sys.argv[2] in ("all", "branches"):
    columns = sys.argv[2]
    arg_start = 3
...
# calls implies callchains in perf script export; ensure call paths are delivered
...
# speed options similar to original script
do_query("PRAGMA journal_mode = OFF")

# schema creation inside a transaction
